...
# ---------- Chapter 1 Quiz (Feature #10) ----------
# Generates multiple-choice questions drawn from the Ch 1 tables so learners
# can practice rasi / nakshatra / tithi / yoga / hora / solar-calendar recall.
# The correct answer is included in the payload — the UI just hides it until
# the user picks; the goal is practice, not scoring.

def _q_nak_lord():
    i = random.randrange(27)
    name, lord, _deity = basics.NAKSHATRAS[i]
    wrong_pool = [l for _, l, _ in basics.NAKSHATRAS if l != lord]
    opts = random.sample(list(set(wrong_pool)), 3) + [lord]
    random.shuffle(opts)
    return {
        "prompt": f"Vimshottari dasa lord of nakshatra #{i + 1}, {name}?",
        "options": opts,
        "correct_index": opts.index(lord),
        "explanation": f"The 27 nakshatras cycle through nine lords (Ketu, Venus, Sun, Moon, Mars, Rahu, Jupiter, Saturn, Mercury). {name} is the {((i % 9) + 1)}-th in the cycle, whose lord is {lord}.",
    }


def _q_nak_deity():
    i = random.randrange(27)
    name, _lord, deity = basics.NAKSHATRAS[i]
    wrong_pool = [d for _, _, d in basics.NAKSHATRAS if d != deity]
    opts = random.sample(list(set(wrong_pool)), 3) + [deity]
    random.shuffle(opts)
    return {
        "prompt": f"Presiding deity of {name} (Ch 1 Table 2)?",
        "options": opts,
        "correct_index": opts.index(deity),
        "explanation": f"{name}'s deity is {deity} (Ch 1 Table 2).",
    }


def _q_rasi_by_number():
    i = random.randrange(12)
    correct = RAASI_NAMES[i]
    wrong = random.sample([n for n in RAASI_NAMES if n != correct], 3)
    opts = wrong + [correct]
    random.shuffle(opts)
    ordinals = ["1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th", "10th", "11th", "12th"]
    return {
        "prompt": f"Which rasi is the {ordinals[i]} sign of the zodiac?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": f"The zodiac runs Aries(1) → Pisces(12); the {ordinals[i]} sign is {correct} ({RAASI_SANSKRIT[i]}).",
    }


def _q_longitude_to_rasi():
    rasi_i = random.randrange(12)
    deg_in = random.randint(0, 29)
    lon = rasi_i * 30 + deg_in
    correct = RAASI_NAMES[rasi_i]
    wrong = random.sample([n for n in RAASI_NAMES if n != correct], 3)
    opts = wrong + [correct]
    random.shuffle(opts)
    return {
        "prompt": f"A longitude of {lon}° falls in which rasi?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": f"Each rasi is 30° wide. {lon}° // 30 = {rasi_i} → {correct} (advance {deg_in}°).",
    }


def _q_tithi_lord():
    # 1..30, lord cycle of 8 starting at Sun for 1st tithi.
    lords = ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn", "Rahu"]
    n = random.randint(1, 30)
    idx = (n - 1) % 8
    correct = lords[idx]
    opts = random.sample([l for l in lords if l != correct], 3) + [correct]
    random.shuffle(opts)
    return {
        "prompt": f"Lord of tithi #{n}?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": f"Tithi lords cycle through 8 planets. Tithi #{n} is position {(idx + 1)} in the cycle → {correct} (Ch 1.3.8 Table).",
    }


def _q_paksha():
    n = random.randint(1, 30)
    correct = "Sukla (waxing)" if n <= 15 else "Krishna (waning)"
    other = "Krishna (waning)" if n <= 15 else "Sukla (waxing)"
    opts = [correct, other, "Both pakshas", "Neither — only Amavasya/Poornima"]
    random.shuffle(opts)
    return {
        "prompt": f"Tithi #{n} belongs to which paksha?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": f"Tithis 1–15 are Sukla paksha (Moon waxing, Moon-Sun = 0° → 180°); 16–30 are Krishna paksha (waning, 180° → 360°).",
    }


def _q_nakshatra_arc():
    opts = ["13°20'", "12°00'", "30°00'", "3°20'"]
    random.shuffle(opts)
    correct = "13°20'"
    return {
        "prompt": "How many degrees wide is one nakshatra?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": "360° / 27 nakshatras = 13°20' per nakshatra. Each splits into 4 padas of 3°20'.",
    }


def _q_pada_arc():
    opts = ["3°20'", "6°40'", "13°20'", "10°00'"]
    random.shuffle(opts)
    correct = "3°20'"
    return {
        "prompt": "How many degrees wide is one pada (quarter of a nakshatra)?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": "A nakshatra is 13°20'; divided into 4 padas, each pada is 3°20'.",
    }


def _q_solar_year_length():
    opts = ["360 days", "365 days", "354 days", "12 × 30 tithis"]
    random.shuffle(opts)
    correct = "360 days"
    return {
        "prompt": "How long is the Vedic solar year (where one day = 1° of Sun's motion)?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": "In the solar calendar, 1 day = 1° of Sun's motion and 1 month = 30°, so the year is exactly 360 solar days — close to, but not equal to, the tropical year.",
    }


def _q_hora_lord():
    # First hora of day N is lord of day N. Day lord order (index 0..6 = Sun..Saturn):
    days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
    weekday_lord = ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn"]
    hora_order = ["Saturn", "Jupiter", "Mars", "Sun", "Venus", "Mercury", "Moon"]
    day_i = random.randrange(7)
    hora_n = random.randint(1, 24)  # 1st..24th hora of the day
    start_lord = weekday_lord[day_i]
    start_idx = hora_order.index(start_lord)
    lord_idx = (start_idx + (hora_n - 1)) % 7
    correct = hora_order[lord_idx]
    opts = random.sample([h for h in hora_order if h != correct], 3) + [correct]
    random.shuffle(opts)
    return {
        "prompt": f"Which planet rules the {hora_n}-th hora of {days[day_i]}?",
        "options": opts,
        "correct_index": opts.index(correct),
        "explanation": f"The 1st hora of {days[day_i]} is ruled by {start_lord}. Subsequent horas cycle through Saturn → Jupiter → Mars → Sun → Venus → Mercury → Moon → Saturn…. Advancing {hora_n - 1} step(s) lands on {correct}.",
    }


_QUIZ_GENERATORS = [
    _q_nak_lord, _q_nak_deity, _q_rasi_by_number, _q_longitude_to_rasi,
    _q_tithi_lord, _q_paksha, _q_nakshatra_arc, _q_pada_arc,
    _q_solar_year_length, _q_hora_lord,
]


@app.get("/api/quiz/next")
async def quiz_next():
    q = random.choice(_QUIZ_GENERATORS)()
